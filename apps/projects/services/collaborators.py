...
class CollaboratorService:
    ...
    async def create_collaborator(self, user, payload):
        ...
        project = await sync_to_async(get_object_or_404)(Project, id=project_id, user=user)

        obj, created = await Collaborator.objects.aget_or_create(defaults={"user": collaborator, "role": role}, project=project, user__email=email)
